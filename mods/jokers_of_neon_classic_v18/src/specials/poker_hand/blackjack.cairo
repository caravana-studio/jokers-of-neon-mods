#[dojo::contract]
pub mod special_blackjack {
    use jokers_of_neon_classic_v18::specials::specials::SPECIAL_BLACKJACK_ID;
    use jokers_of_neon_lib::interfaces::{base::ICardBase, cards::executable::ICardExecutable};
    use jokers_of_neon_lib::models::{
        card_type::CardType, data::card::{Card, Value}, data::poker_hand::PokerHand, tracker::GameContext,
    };

    #[abi(embed_v0)]
    impl BlackjackExecutable of ICardExecutable<ContractState> {
        fn execute(ref self: ContractState, context: GameContext, raw_data: felt252) -> (i32, i32, i32) {
            let mut acum_asc = 0;
            let mut acum_desc = 0;
            let mut cards = context.cards_played;
            loop {
                match cards.pop_front() {
                    Option::Some((
                        _, _, card,
                    )) => {
                        acum_desc = acum_desc + *card.points;

                        acum_asc = if *card.value == Value::Ace {
                            acum_asc + 1
                        } else {
                            acum_asc + *card.points
                        }
                    },
                    Option::None => { break; },
                }
            };

            if acum_asc == 21 || acum_desc == 21 {
                (0, 21, 0)
            } else {
                (0, 0, 0)
            }
        }
    }

    #[abi(embed_v0)]
    impl BlackjackBase of ICardBase<ContractState> {
        fn get_id(self: @ContractState) -> u32 {
            SPECIAL_BLACKJACK_ID
        }

        fn get_types(self: @ContractState) -> Span<CardType> {
            array![CardType::PokerHand].span()
        }
    }
}
